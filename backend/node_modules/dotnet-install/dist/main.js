...
const async_io_2 = require("@microsoft.azure/async-io");
const path = require("path");
var eventing_2 = require("@microsoft.azure/eventing");
exports.Progress = eventing_2.Progress;
...
async function installFramework(version, operatingSystem, architecture, folder = path.normalize(`${os.homedir()}/.dotnet`), force = false, progressInit = () => { }) {
    const ppp = new eventing_1.Progress(progressInit);
    ...
    rq.on("progress", (state) => ppp.NotifyProgress(Math.round(state.percent * 100)));
    rq.on('end', () => ppp.NotifyEnd());
    if (force || !await isInstalled(version, folder)) {
        const release = await async_io_1.Lock.waitForExclusive(folder);
        if (release) {
            await unpack_1.unpack(rq, folder);
            await release();
        }
        throw new exception_1.FrameworkNotInstalledException(folder, version);
    }
}
...
async function listInstalledFrameworkRevisions(folder) {
    if (await async_io_2.exists(folder)) {
        const shared = path.join(folder, "shared", "Microsoft.NETCore.App");
        if (await async_io_2.exists(shared)) {
            // yes there is a shared framework folder. 
            const result = [];
            for (const each of await async_io_2.readdir(shared)) {
